...
public class CsListService  implements CommandProcess {
	...
	public String requestProcess(
		...
		if(! searchOption) {
			...
			csList = dao.csList(startRow, endRow);
			
		} else {
			// 검색어에 해당하는 게시 글 수를 구한다.
			listCount = dao.getCsCount(type, keyword);
			
			// 검색 결과에 대한 현재 페이지에 해당 하는 게시 글 리스트를 DB로부터 읽어온다.
			csList = dao.searchList(type, keyword, startRow, endRow);
		}
